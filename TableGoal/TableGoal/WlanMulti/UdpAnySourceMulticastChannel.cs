...
namespace TableGoal
{
    ...
    public class UdpAnySourceMulticastChannel : IDisposable
    {
        ...
        /// <summary>
        /// Occurs when any socket exception is raised.
        /// </summary>
        ...
        // when exception occurs it is handled and this flag is checked -> then there is no need to handle more exceptions.
        bool currentlyHandlingExpection = false;

        public void ResetExceptionHandlerFlag()
        {
            currentlyHandlingExpection = false;
        }
        ...
        public void SendTo(IPEndPoint endPoint, string format, params object[] args)
        {
            try
            {
                ...
                {
                    DiagnosticsHelper.SafeShow("Please retry multiplayer game.");
                }
            }
            catch (SocketException socketEx)
            {
        ...
        {
            if (currentlyHandlingExpection)
                return;

            currentlyHandlingExpection = true;
            if (socketEx.SocketErrorCode == SocketError.NetworkDown)
            {
                DiagnosticsHelper.SafeShow("Connection problem has occurred. Please make sure your device is on a operational Wi-Fi network.");
            }
            ...
            {
                DiagnosticsHelper.SafeShow("Cannot access Wi-Fi network. Please check your connection.");
            }
            else
            {
                // Just display the message.
                //DiagnosticsHelper.SafeShow(socketEx.Message);
                DiagnosticsHelper.SafeShow("Problem with a Wi-Fi network connection. Match is aborted.");
            }

            // fire exception event, if hooked
            EventHandler handler = this.ConnectionException;
            if (handler != null)
                handler(this, EventArgs.Empty);
        }
    }
